...
public class GridView extends AbsListView {
    ...
    public GridView(Context context, AttributeSet attrs, int defStyle) {
        ...
        int hSpacing = a.getDimensionPixelOffset(R.styleable.GridView_android_horizontalSpacing, 0);
        setHorizontalSpacing(hSpacing);

        int vSpacing = a.getDimensionPixelOffset(R.styleable.GridView_android_verticalSpacing, 0);
        setVerticalSpacing(vSpacing);

        int index = a.getInt(R.styleable.GridView_android_stretchMode, STRETCH_COLUMN_WIDTH);
        if (index >= 0) {
            setStretchMode(index);
        }

        int columnWidth = a.getDimensionPixelOffset(R.styleable.GridView_android_columnWidth, -1);
        if (columnWidth > 0) {
            setColumnWidth(columnWidth);
        }

        int numColumns = a.getInt(R.styleable.GridView_android_numColumns, 1);
    ...
    private View fillSelection(int childrenTop, int childrenBottom) {
        ...
        if (!mStackFromBottom) {
            ...
            ViewUtils.offsetChildrenTopAndBottom(this, offset);
